...
from codes.differential_privacy import utils
...
class DPGradientDescentOptimizer(tf.train.GradientDescentOptimizer):
    ...
    def compute_sanitized_gradients(self, loss, var_list=None,
        ...
        if self.debug:
            print("###INFO: loss_type(%s), loss = %s " % (type(loss), loss))
            input("here 1: <press enter>")
        ...
        sanitized_grads = []
        if self.debug:
            input("here 2: <press enter>")
        for px_grad, v in zip(px_grads, var_list):
            tensor_name = utils.GetTensorOpName(v)
            if self.debug:
                print("#INFO: v=", v)
                print("#INFO: px_grad=", px_grad)
                print("#INFO: _batches_per_lot=", self._batches_per_lot)
                input("here 3: <press enter>")
            ...
            if self.debug:
                input("here 4: <press enter>")
            sanitized_grads.append(sanitized_grad)
        if self.debug:
            input("here x: <press enter>")
    ...
    def minimize(self, loss, global_step=None, var_list=None,
        ...
        if self.debug:
            print(self._batches_per_lot)
        if self._batches_per_lot == 1:
            if self.debug:
                print("###INFO (in _batches_per_lot == 1): loss_type(%s), varlist_type(%s) = %s "%(type(loss), type(var_list), var_list))
            ...
            if self.debug:
                print("###INFO: Gradients And Variables: ", grads_and_vars)
            #SONTN suggests:
            #1. compute sum of absolute values of embeddings's gradients
            for g,v in grads_and_vars:
                print((g.op.name,v.op.name))
                if v.op.name == "embeddings/embeddings":
                    emb_grad = tf.reduce_mean(tf.abs(g))
            #2. return along with apply_grads
            #3. in tf_word_embedding.py when apply to session.run
            #
            self._assert_valid_dtypes([v for g, v in grads_and_vars if g is not None])

            apply_grads = self.apply_gradients(grads_and_vars,
                                               global_step=global_step, name=name)
            return apply_grads, emb_grad
        ...
        if self.debug:
            print("###INFO: update_cond (to accumulate the gradient or actually apply it): ", update_cond)
            input(">>>>>>>")
        ...
        if self.debug:
            print("#DEBUG-INFO: Var list before updating gradients: %s"%(var_list))
            input(">>>>>>>")
